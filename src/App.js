...
function App() {
    ...
    const onEnter = useCallback((ev) => {
        if (ev.keyCode === 10 || ev.keyCode === 13) {
            ...
            setName((name || 'ኢትዮጵያ'));
            setStatus(statusEnum.animating)
        }
    }, [name])
    ...
    const hideAll = useCallback(() => {
        anime({targets: [elements.m1, elements.m2, elements.name, elements.buttons], opacity: 0})
    }, [true])
    useEffect(() => {
        if (status === statusEnum.animating && elements.m1 && elements.m2) {
            anime({
                targets: elements.m1,
                rotate: [0, randomAngle, randomAngle, 0, randomAngle, 0],
                duration: 10000,
                loop: true,
                easing: 'linear'
            })
            anime.timeline().add({
                ...
                .add({
                    targets: elements.buttons,
                    opacity: [0, 1],
                    delay: 1500,
                    complete: setStatus(statusEnum.finished)
                })
        }
    }, [status])
    useEffect(() => {
        if (elements.background && !elements.backgroundStarted) {
            ...
            anime({
                targets: elements.background, keyframes: [
                    ...
                    {
                        scale: 1.3,
                        translateX: getRandomOffsetPercent(),
                        translateY: getRandomOffsetPercent(),
                        duration: 9000,
                        delay: 2000
                    },
                    {scale: 1, translateX: '0%', translateY: '0%', duration: 9000, delay: 2000},
    ...
    return (
        <Stack sx={{justifyContent: 'center', height: '100%', display: 'flex'}}>
            <div style={{position: 'absolute', height: '100%', width: '100%', overflow: 'hidden'}}>
                <img ref={el => elements.background = el}
                     style={{filter: 'blur(4px)', height: '150%', width: '150%', objectFit: 'cover'}}
                     src='./enqutatash.jpg'></img>
            </div>
            <Container sx={{
                padding: '8mm',
                alignItems: 'center',
                ...
                {Array.from(`እንኳን ለአዲሱ ዓመት በሰላም አደረሰን አደረሰ${isMale ? 'ህ' : 'ሽ'}`).map((ch, ind) =>
                    <span key={ind} style={{
                        ...
                        opacity: 0
                    }} ref={(el => el && elements.m1.push(el))}>{ch}</span>
                )}
            </Container>
            <Container sx={{
                padding: '4mm',
                ...
                {Array.from(' ፪፻፲፮ ').map((ch, ind) =>
                    <span key={ind} style={{
                        whiteSpace: 'pre',
                        color: 'brown',
                        fontSize: '2.5rem',
                        fontFamily: 'wookianos',
                        fontWeight: 'bold',
                        display: 'inline-block',
                        opacity: 0,
                        transform: 'translateX(-100%) scale:0.7'
                    }} ref={(el => el && elements.m2.push(el))}>{ch}</span>
                )}
            ...
            <Box sx={{alignItems: 'center', justifyContent: 'center', display: 'flex'}}>
                ...
                }} ref={(el) => elements.name = el}>{`ለ${name}`}</h1>
            </Box>
            <Stack ref={el => elements.buttons = el} direction={'row'} spacing={3}
                   sx={{justifyContent: 'center', padding: '8mm', opacity: 0, color: 'white'}}>
                <IconButton onClick={() => {
                    if (status === statusEnum.finished) {
                        hideAll();
                        setStatus(statusEnum.animating)
                    }
                }}><ReplaySharp></ReplaySharp> </IconButton>
                <IconButton onClick={() => {
                    if (status === statusEnum.finished) {
                        hideAll();
                        setStatus(statusEnum.dialog)
                    }
                }}><Person2Rounded/></IconButton>

            </Stack>
            <Dialog open={status == statusEnum.dialog}>
                <DialogContent>
                    <Stack direction="column">
                        <TextField style={{border: 'goldenrod', outline: 'goldenrod', fontFamily: 'wookianos'}}
                                   value={name} variant='outlined' placeholder='ስምዎትን ያስገቡ' lang='amh' onChange={onEdit}
                                   onKeyDown={onEnter}></TextField>
                        <Stack direction={'row'} sx={{
                            ...
                            <Button style={{backgroundColor: '#FAE033', color: 'white', fontFamily: 'wookianos'}}
                                    onClick={toggleGender}> {isMale ? 'ወንድ' : 'ሴት'}</Button>
                        ...
                        <Typography
                            variant='body2'><a style={{textDecoration: 'none', color: '#bbbbbb'}}
                                               href="mailto:yonatha12345678910@gmail.com">@yoni_ash</a></Typography>
                    </Stack>
                </DialogContent>
            </Dialog>
        </Stack>
